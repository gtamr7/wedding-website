...
export default function Travel() {
  ...
  return (
    <section id="travel" className="section-py bg-ivory px-6">
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <h2 className="font-display text-5xl sm:text-6xl italic text-charcoal">Travel & Stay</h2>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          <div className="space-y-10">
            ...
            <FadeIn>
              <div>
                <h3 className="font-display text-2xl italic text-charcoal mb-1">The Venue</h3>
                <div className="gold-divider w-12 mb-4" />
                <p className="text-charcoal font-medium">Powel Crosley Estate</p>
                <p className="text-charcoal/60 text-sm mt-1">8374 N Tamiami Trail, Sarasota, FL 34243</p>
                ...
                <p className="text-xs text-charcoal/40 mt-2">Ample parking on-site · Valet available</p>
              </div>
            </FadeIn>
            ...
            <FadeIn delay={0.1}>
              <motion.div
                ...
                className="relative rounded-xl overflow-hidden border border-olive-light shadow-sm aspect-video bg-olive-light/30"
              >
                <iframe
            ...
            {/* Weather */}
            <FadeIn delay={0.15}>
              <div className="flex items-start gap-3 bg-olive-light/30 rounded-xl p-4 border border-olive-light">
                <span className="text-2xl" aria-hidden="true">☀️</span>
                <div>
                  <p className="font-medium text-charcoal text-sm">February in Sarasota</p>
          ...
          <div className="space-y-10">
            ...
            <FadeIn delay={0.05}>
              <div>
                <h3 className="font-display text-2xl italic text-charcoal mb-1">Nearest Airports</h3>
                  ...
                  {airports.map((a) => (
                    <div
                      ...
                      className={`flex items-center justify-between p-3 rounded-lg border ${
                        a.recommended
                          ? 'bg-olive-light/30 border-olive-mid/40'
                          : 'bg-white border-olive-light'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <span className="font-display text-xl font-semibold text-olive-mid w-12">{a.code}</span>
                        <div>
                          <p className="text-sm text-charcoal leading-snug">{a.name}</p>
                          {a.recommended && (
                            <p className="text-[10px] uppercase tracking-wider text-gold mt-0.5">Recommended</p>
                          )}
                        </div>
                      </div>
                      <span className="text-sm text-charcoal/50 whitespace-nowrap ml-2">{a.distance}</span>
                    </div>
                  ))}
                </div>
            ...
            <FadeIn delay={0.1}>
              <div>
                <h3 className="font-display text-2xl italic text-charcoal mb-1">Where to Stay</h3>
                <div className="gold-divider w-12 mb-5" />
                <div className="space-y-3">
                  {hotels.map((h) => (
                    <div
                      key={h.name}
                      className="p-4 rounded-lg border border-olive-light bg-white hover:border-olive-mid/60 hover:shadow-sm transition-all duration-200"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <p className="font-medium text-charcoal text-sm leading-snug">{h.name}</p>
                        <span className="text-[10px] uppercase tracking-wider text-olive-mid whitespace-nowrap shrink-0 border border-olive-mid/30 rounded-full px-2 py-0.5">
                          {h.type}
                        </span>
                      </div>
                      <p className="text-xs text-charcoal/50 mt-1">📍 {h.distance}</p>
                      <p className="text-xs text-charcoal/60 mt-1">{h.note}</p>
                    </div>
                  ))}
                </div>
              </div>
